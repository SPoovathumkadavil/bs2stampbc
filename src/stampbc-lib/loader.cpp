...
Loader* Loader::m_pInstance = 0;
const int Loader::packetSize = 18;

const StampID Loader::m_stampID[] = {
  {"BS2", "",         STAMPID_BS2,   0},
  {"e",   "e",        STAMPID_BS2E,  8},
...
// setPort
//------------------------------------------------------------------------------
void Loader::setPort(std::string sPort)
{
  m_port = sPort;
}
...
// typeToIndex
//------------------------------------------------------------------------------
int Loader::typeToIndex(const std::string& sType)
{
  int i = 0;
...
// versionToVersionID
//------------------------------------------------------------------------------
std::string Loader::versionToVersionID(const std::string& sVer)
{
  int i(0);
  ...
  return m_versionID[i][0] ? std::string(m_versionID[i][1]) : sVer;
}

//------------------------------------------------------------------------------
// identify
//------------------------------------------------------------------------------
bool Loader::identify(std::string& sType, std::string& sVersion, int& index)
{
  ...
  if (sType == "") {
    if (once) {
      CERR << "trying to identify the stamp" << std::endl;
      once = false;
    }
    ...
    for (int i = 0; m_stampID[i].send; i++) {
      std::string type(m_stampID[i].id);
      std::string version;
    ...
    if (once) CERR << "identifying the stamp" << std::endl;
  ...
  if (m_done) {
    ...
    if (m_done) {
      sType = std::string(m_stampID[index].id);
...
// fileSize
//------------------------------------------------------------------------------
long Loader::fileSize(const std::string fileName)
{
  struct stat sbuf;
...
// download
//------------------------------------------------------------------------------
bool Loader::download(const std::string objFile, std::string& sType, int slot)
{
  ...
  if (slot < 0) {
    CERR << "downloading " << objFile << std::endl;
  }
  else {
    CERR << "downloading " << objFile << " to slot " << slot << std::endl;
  }
...
// waitRecv
//------------------------------------------------------------------------------
void Loader::waitRecv(bool bQuiet)
{
  if FP_SAFE {
    fd_set rfds;
    struct timeval tv;
...
// receive
//------------------------------------------------------------------------------
void Loader::receive(const char* pBuffer, int n, int& nRet, bool bQuiet)
{
  if FP_SAFE {
    nRet = 0;
    int n0(n);
...
// send
//------------------------------------------------------------------------------
void Loader::send(const char* pBuffer, int n)
{
  if FP_SAFE {
...
// sendBreakCondition
//------------------------------------------------------------------------------
void Loader::sendBreakCondition()
{
  if FP_SAFE {
    char buf[64];
    int n;
...
// sendIdChars
//------------------------------------------------------------------------------
void Loader::sendIdChars(const char* toSend, const char* toRecv,
  std::string& sVer, 
  bool isBS2)
{
  if FP_SAFE {
...
// downloadPacket
//------------------------------------------------------------------------------
void Loader::downloadPacket(const char* pBuffer, int packnum)
{
  if (FP_SAFE) {
